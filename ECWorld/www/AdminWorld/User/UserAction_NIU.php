...
function getUsersByParent($mysqlObj,$lang){
	...
	$userObj=new b_users($userDetails->user,$mysqlObj,$lang);
	//$roles = explode(',',$roleIDs);
	echo json_encode($userObj->getUsersByParentID($parentID,$includeParent,$excludeRoleIDs));
}
...
function Upsert($mysqlObj,$lang,$langSMS){	
	...
	$bReqObj = $bReqObj->giveMeObj($loggedInUserDetails->user->UserID,$loggedInUserDetails->user->Mobile,$_SERVER["REMOTE_ADDR"],"WEB","000000","0","WEB REQUEST","Add/Update User","1",$today,$today,"WEB REQUEST");
	if($_POST['UserID']==0){
