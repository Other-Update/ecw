...
function addTransfer($mysqlObj,$lang,$langSMS){	
	...
	$bReqObj = $bReqObj->giveMeObj($loggedInUserDetails->user->UserID,$loggedInUserDetails->user->Mobile,$_SERVER["REMOTE_ADDR"],"WEB","000000","0","WEB REQUEST","Add Payment","1",$today,$today,"WEB REQUEST");
	...
	if($res->isSuccess){
		...
		$bSMSObj->paymentTransfer(true,"Payment_s",$pObj->ToUserID,$pObj->Amount,0,0,$bReqObj);
	}else{
		$bReqObj->Status="4";
		$bReqObj->Remark="Failed to transfer";
		$bSMSObj->paymentTransfer(true,"Payment_f",$pObj->ToUserID,$pObj->Amount,0,0,$bReqObj);
	}
